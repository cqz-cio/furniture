...
$sql = @"
select
 (select count(*) from product_sku s join product_spu p on p.id=s.spu_id and p.tenant_id=s.tenant_id and p.deleted=b'0' where s.tenant_id=121 and s.deleted=b'0' and p.creator='furniture-agent-seed' and p.status=1) active_mall_skus,
 (select count(*) from erp_product where tenant_id=121 and creator='mall-erp-seed' and deleted=b'0') erp_products,
 (select count(distinct mall_sku_id) from mall_erp_product_mapping where tenant_id=121 and deleted=b'0') unique_mappings,
...
if ($values.Count -ne 6) { throw "Unexpected audit result: $output" }
$result = [ordered]@{ active_mall_skus=[int]$values[0]; erp_products=[int]$values[1]; unique_mappings=[int]$values[2]; orphan_mappings=[int]$values[3]; cross_tenant_mappings=[int]$values[4]; stock_without_warehouse=[int]$values[5] }
$result.GetEnumerator() | ForEach-Object { Write-Host ("audit {0}={1}" -f $_.Key,$_.Value) }
if ($result.active_mall_skus -ne 26 -or $result.erp_products -ne 26 -or $result.unique_mappings -ne 26 -or $result.orphan_mappings -ne 0 -or $result.cross_tenant_mappings -ne 0 -or $result.stock_without_warehouse -ne 0) { throw "Tenant 121 mall ERP audit failed." }
