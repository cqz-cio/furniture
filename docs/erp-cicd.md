...
两套 CD 初始均为手动触发，且要求相应 GitHub Environment 的 `ERP_CD_ENABLED=true`。生产新部署要求同一份发布清单最近一次测试 CD 成功；生产回滚只允许选择本机登记为历史成功、仍受保护且数据库兼容的版本，不依赖测试环境此刻是否正常。

清理默认 dry-run，实际删除还要求仓库变量 `ERP_IMAGE_CLEANUP_ENABLED=true`。定时入口建议在真实 dry-run 和首轮清理核对后开启；变量未开启时定时任务不执行。工作流默认北京时间每天 03:30 检查。

本次代码实现没有连接服务器部署、没有修改 GitHub 变量或凭据，也没有执行镜像删除。公司的第三个官网不在这次发布范围。原有 `ERP test environment check` 仍保留为旧环境的只读诊断；新 CD 自带容器验收，不能把旧检查的通过结果当成新版本发布验收。
...
本脚本用于完成首次接入之后的日常发布，不自动改造一台仍在运行旧 systemd 服务或旧数据库迁移账本的机器。测试环境历史文档记录为 V047、Flyway 未开启、使用 `oakved-yudao.service`，这些必须先重新查实；不能据此推断今天的生产状态。

先在测试副本上完成恢复演练和现有 Flyway 账本接管验证，再安排维护窗口完成旧服务退出与首次 Docker 切换。不要手工伪造迁移历史，也不要向既有库导入初始化基线。确认测试与生产使用不同业务库及适当隔离的 Redis/文件存储。
...
数据库恢复不会自动执行；导回旧备份可能覆盖后来产生的业务数据。`manual-recovery-required` 或 SSH 中断留下的 `in_progress` 状态需先核对真实容器和数据库，再处置记录；不能直接删除 state.json 后反复重跑。备份、磁盘测量和业务恢复演练记录由部署负责人维护。
...
另外校验 GitHub Actions 语法、两个 shell 入口、Compose 的两个应用服务/回环绑定/原始秘密值/日志设置。首次真实构建发布、SSH 部署、代理路由、MySQL 恢复演练和实际 GHCR 删除仍需在配置好的测试环境执行，不能把本地检查写成线上已经部署成功。
