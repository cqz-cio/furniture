...
每阶段有有限超时和进度输出，命令 60 秒无进展会终止进程组。根据镜像压缩/解包大小核算容量，保留至少 10 GiB 余量，占用超过 90% 停止。GHCR 继续保留 **5 套普通版本**，额外保护当前/回滚/pin；生产未接入时保持清理关闭。
...
这不表示新工作流完整 `prepare` 或 `cutover` 已实际执行。真实镜像下载、完整隔离迁移和 Docker 切换，以新工作流对应运行记录为准。
