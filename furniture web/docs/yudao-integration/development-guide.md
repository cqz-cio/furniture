...
所有新增模块、页面、弹窗、表单、按钮、提示、错误信息、placeholder、aria label 和其他用户可见文案，都必须同步完成多语言适配。

- 用户可见文本必须先放进 `src/i18n.js`，组件里只允许通过 `t("...")` 使用文案，不要直接硬编码英文、中文或法文。
- 每个新增文案 key 必须同时补齐 `en`、`zh-CN`、`fr` 三套语言版本。
- 新增模块的结构测试或 i18n 测试必须覆盖关键文案 key，确保三种语言都能取到值。
- 如果修改已有模块时发现硬编码文案，应在本次改动范围内一并迁移到 `src/i18n.js`。
- 验收时至少运行 `npm.cmd test`；涉及新页面或关键弹窗时，还要检查语言切换后页面不再残留硬编码英文。
