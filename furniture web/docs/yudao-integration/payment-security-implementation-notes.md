...
# 2026-06-17 Review 补齐项

1. 支付关闭回调现在和成功回调一样先校验 `channelId + channelCode`，错误渠道不能关闭其他渠道创建的支付扩展单。
2. `submitOrder` 里渠道同步返回成功时，只吞掉可判定为幂等或并发的状态异常；金额、渠道、渠道交易号等安全校验异常会继续抛出，避免外部已扣款但内部仍停在待支付。
3. `pay_order` 增加 `channel_id + channel_order_no` 唯一索引，并在成功入账更新时把数据库唯一键冲突映射为 `PAY_ORDER_NOTIFY_CHANNEL_ORDER_NO_CONFLICT`。
4. 测试库 DDL 同步增加唯一索引，`PayOrderServiceTest` 已覆盖关闭回调渠道错配、同步成功安全异常透传、渠道交易号数据库唯一约束。
