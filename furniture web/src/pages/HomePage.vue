...
<template>
  ...
  <section class="home-grid" :aria-label="t('home.gridAria')">
    <a v-for="item in editorialModules" :key="item.title" :href="item.href" class="home-entry">
      <picture class="home-entry-picture">
        <source media="(max-width: 760px)" :srcset="item.mobile" />
        <img class="home-entry-image" :src="item.desktop" :alt="item.alt" />
        <span class="home-entry-shade" aria-hidden="true"></span>
      </picture>
      <div class="home-entry-copy">
        <BrandEyebrow :suffix="item.eyebrow" />
        <h2>{{ item.title }}</h2>
        <p>{{ item.copy }}</p>
        <span>{{ item.cta }}</span>
      </div>
