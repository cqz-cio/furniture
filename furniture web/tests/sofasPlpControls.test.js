...
describe("sofas PLP browse controls", () => {
  it("wires search, type filter, sort, counts, and empty recovery into the product grid", () => {
    ...
    expect(source).toContain("const selectedProductType = ref(legacyRoomFilters[initialListingQuery.filter]");
    expect(source).toContain("getProductCategoryTree(requestOptions)");
    expect(source).toContain("roomCatalog.value = buildRoomCatalog(categoryTree)");
    expect(source).toContain("allowedTypes: selectedRoom.value?.productTypes");
    ...
    expect(source).toContain("const hasProductPrice = (product) =>");
    expect(source).toContain("const hasProductInventory = (product) =>");
    expect(source).toContain('v-if="hasProductPrice(product)"');
    expect(source).toContain('v-if="product.badge || hasProductInventory(product)"');
