...
$mallSkuCount = [int](Invoke-MySql "select count(*) from product_sku s join product_spu p on p.id=s.spu_id and p.tenant_id=s.tenant_id and p.deleted=b'0' where s.tenant_id=$TenantId and s.deleted=b'0' and p.creator='furniture-agent-seed' and p.status=1;")
if ($mallSkuCount -ne 26) { throw "Expected $ExpectedSkuCount active tenant-$TenantId mall SKUs, found $mallSkuCount." }
...
JOIN erp_product_category c ON c.tenant_id=$TenantId AND c.code='FURNITURE' AND c.deleted=b'0'
JOIN erp_product_unit u ON u.tenant_id=$TenantId AND u.name='Piece' AND u.deleted=b'0'
WHERE s.tenant_id=$TenantId AND s.deleted=b'0' AND p.creator='furniture-agent-seed' AND p.status=1
...
$mappingCount = [int](Invoke-MySql "select count(*) from mall_erp_product_mapping where tenant_id=$TenantId and deleted=b'0';")
