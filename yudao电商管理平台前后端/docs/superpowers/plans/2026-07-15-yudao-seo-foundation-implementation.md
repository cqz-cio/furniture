...
- V021 中依赖 `deleted` 的唯一键只是过渡结构；最终唯一性契约由 V022 的 nullable generated `active_record` 实现，只允许一个 active 业务键，同时允许任意条 deleted 历史。公开接口只返回 `PUBLISHED` 状态。
...
- Create: `yudao-cloud/sql/mysql/migrations/V021__seo_foundation.sql`
- Create: `yudao-cloud/sql/mysql/migrations/V022__seo_active_record_uniqueness.sql` (append-only forward migration; V021 remains immutable)
...
- Create: `yudao-cloud/sql/mysql/migrations/V021__seo_foundation.sql`
- Create in Task 4: `yudao-cloud/sql/mysql/migrations/V022__seo_active_record_uniqueness.sql` (append-only; do not rewrite committed V021)
- Create: `yudao-cloud/yudao-module-seo/yudao-module-seo-server/src/test/resources/sql/create_tables.sql`
- Create: `yudao-cloud/yudao-module-seo/yudao-module-seo-server/src/test/resources/sql/clean.sql`
...
`V021__seo_foundation.sql` creates the base schema. 当前分支的履约迁移占用 V015-V020，因此 SEO 从 V021 开始；V021 提交后保持不可变，Task 4 通过 V022 追加修正：
...
```powershell
mvn -pl yudao-module-seo/yudao-module-seo-api -am -Dtest=SeoEnumTest -Dsurefire.failIfNoSpecifiedTests=false test
Select-String -Path sql/mysql/migrations/V021__seo_foundation.sql -Pattern "seo_site_config","seo_metadata","seo:metadata:publish"
```
...
git add -f -- yudao电商管理平台前后端/yudao-cloud/sql/mysql/migrations/V021__seo_foundation.sql
...
```powershell
Select-String -Path sql/mysql/migrations/V021__seo_foundation.sql -Pattern "seo_site_config","seo_metadata","uk_tenant_site_deleted","uk_entity_locale_deleted","idx_public_resolve"
Select-String -Path sql/mysql/migrations/V022__seo_active_record_uniqueness.sql -Pattern "DROP INDEX","uk_tenant_site_deleted","uk_entity_locale_deleted","active_record","uk_tenant_site_active","uk_entity_locale_active"
...
- V021 contains the base tables/transitional indexes, while V022 drops both transitional indexes and supplies both generated columns and replacement active-only unique indexes; `SeoMigrationContractTest` and `SeoSchemaSqlTest` verify the SQL contract and both tables' duplicate-active/delete-recreate behavior;
...
当前分支已包含 V015-V020 的履约迁移，SEO V021/V022 可由自定义顺序迁移器连续执行。本验证仍是 SQL 契约与顺序审计；这些文件由项目的自定义迁移脚本执行，而不是 Flyway。
