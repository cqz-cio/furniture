...
`V019__seo_foundation.sql` creates the base schema. In the integrated migration catalog, fulfillment owns V015-V018 and SEO follows with V019-V020; V015-V018 remain immutable. After V019 is committed it remains immutable; Task 4 adds V020 as an append-only forward correction:
...
The merged catalog now contains the fulfillment-owned V015-V018 migrations, followed contiguously by SEO V019/V020. This verification is a SQL-contract/order audit, not a real MySQL/Flyway end-to-end migration pass; these files are run by the custom migration script rather than Flyway.
