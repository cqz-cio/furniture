...
CREATE TABLE IF NOT EXISTS `product_statistics` (
  ...
  `browse_user_count` bigint NOT NULL DEFAULT 0,
  `favorite_count` bigint NOT NULL DEFAULT 0,
  `cart_count` bigint NOT NULL DEFAULT 0,
  `order_count` bigint NOT NULL DEFAULT 0,
  `order_pay_count` bigint NOT NULL DEFAULT 0,
...
SET @ddl = IF((SELECT COUNT(*) FROM information_schema.COLUMNS WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='product_statistics' AND COLUMN_NAME='trade_watermark')=0, 'ALTER TABLE `product_statistics` ADD COLUMN `trade_watermark` datetime(3) DEFAULT NULL', 'SELECT 1'); PREPARE s FROM @ddl; EXECUTE s; DEALLOCATE PREPARE s;
SET @ddl = IF((SELECT COUNT(*) FROM information_schema.COLUMNS WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='product_statistics' AND COLUMN_NAME='refund_watermark')=0, 'ALTER TABLE `product_statistics` ADD COLUMN `refund_watermark` datetime(3) DEFAULT NULL', 'SELECT 1'); PREPARE s FROM @ddl; EXECUTE s; DEALLOCATE PREPARE s;
