...
-- This development branch has an independent V001-V030 catalog. When integrating into
-- main (already at V034), this migration must be renumbered to the next main version.
...
SELECT
  ...
  'V030-single-role', CURRENT_TIMESTAMP, 'V030-single-role', CURRENT_TIMESTAMP, b'0', 162
FROM `system_tenant`
WHERE `id` = 162
  AND `deleted` = b'0'
  AND NOT EXISTS (
...
SET role_menu.`deleted` = b'1',
    role_menu.`updater` = 'V030-single-role',
...
SELECT
  ...
  'V030-single-role', CURRENT_TIMESTAMP, 'V030-single-role', CURRENT_TIMESTAMP, b'0', 1
