...
class BehaviorEventServiceImplTest {
    ...
    @Test void acceptedPublicEvent_isHashedAndInserted() {
        ...
        service.trackPublic(request(BehaviorEventTypeEnum.HOME_VIEW.getValue(),true),"visitor","session","203.0.113.9",null);
    ...
    @Test void publicAddAndMissingConsent_areRejected() {
        assertThrows(IllegalArgumentException.class,()->service.trackPublic(request(3,true),"v","s","203.0.113.9",null));
        assertThrows(IllegalArgumentException.class,()->service.trackPublic(request(1,false),"v","s","203.0.113.9",null));
    ...
    @Test void redisFailure_failsClosedAndRecordsAnonymousGap() {
        ...
        assertThrows(IllegalStateException.class,()->service.trackPublic(request(1,true),"secret-v","secret-s","203.0.113.9",null));
    ...
    @Test void exceededRateLimit_returns429SemanticAndDoesNotInsert() {
        ...
        assertThrows(BehaviorEventServiceImpl.BehaviorRateLimitException.class,
                ()->service.trackPublic(request(1,true),"v","s","203.0.113.9",null));
        verify(eventMapper,never()).insert(any(BehaviorEventDO.class));
        verifyNoInteractions(gapService);
    }
    private AppBehaviorEventTrackReqVO request(int type,boolean consent){return new AppBehaviorEventTrackReqVO().setEventId("evt-1").setEventType(type).setPagePath("/?token=secret").setConsentGranted(consent);}
}
