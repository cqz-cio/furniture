...
class BehaviorTrackingConsentPolicyTest {
 ...
 @Test void consentRequired_rejectsMissingEvidenceAndAcceptsVerifiedHeaders(){
  ...
  request.addHeader("x-analytics-consent-evidence","signed-cmp-evidence"); request.addHeader("x-analytics-visitor-id","v"); request.addHeader("x-analytics-session-id","s");
  assertTrue(policy.currentDecision(9L).isAllowed());
  assertEquals("signed-cmp-evidence",policy.currentDecision(9L).getConsentEvidence());
 }
