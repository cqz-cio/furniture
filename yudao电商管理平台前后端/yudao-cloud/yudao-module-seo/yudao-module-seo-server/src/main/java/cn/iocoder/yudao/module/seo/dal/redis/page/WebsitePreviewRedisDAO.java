...
@Repository
public class WebsitePreviewRedisDAO {
    ...
    private String key(String kind, String token) { return "website-site:preview:" + kind + ":" + DigestUtil.sha256Hex(token); }
    public void setTicket(String token, WebsitePreviewGrant grant) {
        stringRedisTemplate.opsForValue().set(key("ticket", token), JsonUtils.toJsonString(grant), Duration.ofMinutes(2));
    }
    public WebsitePreviewGrant consumeTicket(String token) {
        String value = stringRedisTemplate.execute(CONSUME, List.of(key("ticket", token)));
        return value == null ? null : JsonUtils.parseObject(value, WebsitePreviewGrant.class);
    }
    public void setSession(String token, WebsitePreviewGrant grant) {
        stringRedisTemplate.opsForValue().set(key("session", token), JsonUtils.toJsonString(grant), Duration.ofMinutes(15));
    }
    public WebsitePreviewGrant getSession(String token) {
